...
def corpo(c):
    ...
    if c['rascunho']:
      ...
      <span data-pt>As imagens são reais, extraídas do projeto. Ano, prazo, resultados e
        depoimento ainda são provisórios e estão marcados na página. Ela está com noindex
        e fora da grade de projetos até o conteúdo fechar.</span>
      <span data-en>The images are real, taken from the project. Year, timeline, results
        and testimonial are still provisional and marked on the page. It carries noindex
        and stays out of the projects grid until the content is final.</span>
    </div>
'''
...
.cw-rascunho{background:#0A0B0D;color:#A0A4AD;border-bottom:1px solid #23262C;
  padding:14px 24px;font-size:13.5px;line-height:1.5;text-align:center}
.cw-rascunho b{color:#F07A65;display:block;font-family:"Bricolage Grotesque",system-ui,sans-serif;
  font-size:13px;text-transform:uppercase;letter-spacing:.1em;margin-bottom:4px}
.cw-rascunho span[data-pt],.cw-rascunho span[data-en]{max-width:70ch;display:inline-block}
